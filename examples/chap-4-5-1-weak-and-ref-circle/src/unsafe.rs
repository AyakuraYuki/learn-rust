...
impl Tree {
    ...
    pub fn successor(&self, data: i32) -> Option<i32> {
        unsafe {
            let node = find_node(self.root, data);
            if !node.is_null() {
                let node_succ = successor_of_node(node);
                if !node_succ.is_null() {
                    return Some((*node_succ).data);
                }
            }
            None
        }
    ...
    fn remove_node(&mut self, node: *mut Node) {
        unsafe {
            ...
            if left_child.is_null() && right_child.is_null() {
            ...
            } else if !left_child.is_null() && !right_child.is_null() {
                ...
                // 节点只有左子节点，所以使用后者替代前者
                self.replace_node(node, left_child);
            } else if !right_child.is_null() {
                // 节点只有右子节点，所以使用后者替代前者
                self.replace_node(node, right_child);
            } else {
                panic!("unreachable");
            }
        }
...
fn find_node(from_node: *mut Node, data: i32) -> *mut Node {
    unsafe {
        if from_node.is_null() || (*from_node).data == data {
            from_node
        } else if data < (*from_node).data {
            find_node((*from_node).left, data)
        } else {
            find_node((*from_node).right, data)
        }
    }
}
